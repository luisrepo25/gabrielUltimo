...
<div class="animate-fade-up" style="max-width: 600px; margin: 0 auto;">
    ...
    <div class="form-container">
        <h2 style="margin: 0 0 24px 0;">Editar Categoría: {{ $categoria->nombre }}</h2>
        ...
        <form action="{{ route('admin.categorias.update', $categoria->idcategoria) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid" style="grid-template-columns: 1fr; gap: 20px;">
                ...
                <div class="field">
                    <label>ID</label>
                    <input type="text" value="{{ $categoria->idcategoria }}" disabled
                ...
                <div class="field">
                    ...
                    <textarea name="descripcion" rows="3" maxlength="255"
                              style="width: 100%; box-sizing: border-box; resize: vertical;">{{ old('descripcion', $categoria->descripcion) }}</textarea>
                ...
                <div class="field">
                    ...
                    <select name="id_categoria_padre" style="width: 100%; box-sizing: border-box; padding: 10px 12px; border: 1px solid var(--border); border-radius: 10px; background: white;">
                        ...
                        @foreach($padres as $padre)
                            <option value="{{ $padre->idcategoria }}"
                                {{ old('id_categoria_padre', $categoria->id_categoria_padre) == $padre->idcategoria ? 'selected' : '' }}>
                                {{ $padre->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="field">
                    ...
                    @else
                        <p style="color: var(--muted); font-size: 0.9rem;">Sin imagen cargada.</p>
                    @endif
                    <label style="margin-top: 8px; display: block;">Nueva imagen (opcional)</label>
                    <input type="file" name="imagen" accept="image/*" style="width: 100%; box-sizing: border-box;">
                    <small style="color: var(--muted);">Dejar vacío para conservar la imagen actual.</small>
                ...
                <div class="field" style="display: flex; align-items: center; gap: 12px;">
                    <input type="checkbox" name="estado" value="1" id="estado"
                           {{ old('estado', $categoria->estado) ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
            ...
            <div style="margin-top: 28px; display: flex; gap: 12px;">
                <button type="submit" class="btn-action">Actualizar Categoría</button>
