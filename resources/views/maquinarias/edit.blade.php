...
<div class="admin-card animate-fade-in" style="padding: 24px; margin-top: 20px; background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 600px; margin-left: auto; margin-right: auto;">
    <div style="margin-bottom: 24px;">
        <h1 style="font-size: 1.8rem; font-weight: 700; color: #1e293b; margin: 0;">Editar Maquinaria</h1>
        <p style="font-size: 0.9rem; color: #64748b; margin-top: 4px;">Modifique los detalles de la maquinaria: {{ $maquinaria->nombre }}.</p>
    </div>
    ...
    <form action="{{ route('maquinarias.update', $maquinaria->id) }}" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
            <div>
                ...
                <input type="text" name="codigo" id="codigo" value="{{ old('codigo', $maquinaria->codigo) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#cbd5e1'">
            </div>
            <div>
                ...
                <select name="estado" id="estado" required style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; background: white; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#cbd5e1'">
                    ...
                    <option value="alquilado" {{ old('estado', $maquinaria->estado) == 'alquilado' ? 'selected' : '' }} {{ $maquinaria->estado == 'alquilado' ? '' : 'disabled' }}>Alquilado (Solo vía Alquileres)</option>
        ...
        <div style="margin-bottom: 16px;">
            ...
            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $maquinaria->nombre) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#cbd5e1'">
        </div>
        ...
        <div style="margin-bottom: 16px;">
            ...
            <textarea name="descripcion" id="descripcion" rows="3" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; resize: vertical; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#cbd5e1'">{{ old('descripcion', $maquinaria->descripcion) }}</textarea>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 24px;">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="precio_dia" id="precio_dia" value="{{ old('precio_dia', $maquinaria->precio_dia) }}" min="0" required style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#cbd5e1'">
            </div>
            <div>
                ...
                <input type="number" step="0.01" name="garantia_sugerida" id="garantia_sugerida" value="{{ old('garantia_sugerida', $maquinaria->garantia_sugerida) }}" min="0" required style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#cbd5e1'">
            </div>
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px;">
            ...
            <button type="submit" class="btn-primary" style="padding: 10px 20px; border-radius: 8px; border: none; color: white; background: var(--primary); font-weight: 600; cursor: pointer; transition: background 0.2s;">Actualizar Maquinaria</button>
        </div>
    </form>
</div>
@endsection
